...
- Philosophy: Unix "do one thing well" — `aural` itself is the verb (capture/transcribe/transcode via flags); only `devices`/`apps`/`info` (and planned `models`) are subcommands. stdout/stdin streaming, POSIX conventions
