...
"$AURAL" -i "$WORK/speech.aiff" 2>/dev/null > "$WORK/out.txt"
check "aiff -> txt" "$WORK/out.txt"
"$AURAL" -i "$WORK/speech.aiff" --transcript-format srt 2>/dev/null > "$WORK/out.srt"
grep -q -- "-->" "$WORK/out.srt" && check "aiff -> srt" "$WORK/out.srt"
"$AURAL" -i "$WORK/speech.aiff" --transcript-format json 2>/dev/null > "$WORK/out.json"
...
for fmt in wav m4a flac; do
    "$AURAL" -i "$WORK/speech.aiff" -a "$WORK/speech.$fmt"
    "$AURAL" -i "$WORK/speech.$fmt" 2>/dev/null > "$WORK/out-$fmt.txt"
    check "$fmt" "$WORK/out-$fmt.txt"
done

echo "== stdin: WAV stream pipe"
cat "$WORK/speech.wav" | "$AURAL" -i - 2>/dev/null > "$WORK/out-pipe.txt"
check "wav pipe" "$WORK/out-pipe.txt"

echo "== stdin: raw PCM pipe"
"$AURAL" -i "$WORK/speech.aiff" -a "$WORK/speech16.wav" -r 16000 -c 1
...
cat "$WORK/speech16.pcm" \
    | "$AURAL" -i - --input-rate 16000 --input-channels 1 2>/dev/null \
    > "$WORK/out-raw.txt"
check "raw pcm pipe" "$WORK/out-raw.txt"

echo "== engine failure propagates non-zero exit (US03)"
echo "not a model" > "$WORK/fake-model.bin"
if "$AURAL" -i "$WORK/speech.wav" --model "$WORK/fake-model.bin" 2>/dev/null; then
